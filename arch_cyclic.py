...
class neuro(nn.Module):
    ...
    def forward(self, f1,f2,f3,hid,x_out):
        f1_cyc = torch.cat((f1[:, :, :, f1.shape[3]//2:],f1[:, :, :, f1.shape[3]//2:]),dim=3)
        f2_cyc = torch.cat((f2[:, :, :, f2.shape[3]//2:],f2[:, :, :, f2.shape[3]//2:]),dim=3)
        f3_cyc = torch.cat((f3[:, :, :, f3.shape[3]//2:],f3[:, :, :, f3.shape[3]//2:]),dim=3)

        local_feat_normal = F.relu(self.feats(f1,f2,f3)) 
        feat_cyclic = F.relu(self.feats(f1_cyc,f2_cyc,f3_cyc))
        local_feat_cyclic = torch.cat((feat_cyclic[:, :, :, feat_cyclic.shape[3]//2:],feat_cyclic[:, :, :, feat_cyclic.shape[3]//2:]),dim=3)
        
        local_feat = local_feat_cyclic + local_feat_normal  
